...
vec3 calcDirectionLisht(DirectionLight light, vec3 normal, vec3 viewDir)
{
    ...
    vec3 diffuse = max(dot(lightDir, normal), 0.0) * light.diffuse * vec3(texture(material.diffuseTex, TexCoor));

    vec3 reflectDir = reflect(light.direction, normal);
    vec3 specular = pow(max(dot(reflectDir, viewDir), 0.0), material.shinnese) * light.specular * vec3(texture(material.specularTex, TexCoor));
...
void main()
{
    ...
    vec3 result = calcDirectionLisht(directionLight, norm, viewDir);
